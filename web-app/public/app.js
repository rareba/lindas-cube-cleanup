...
function getQueryTemplates(graphUri, cubeUri) {
    return {
        ...
        'delete-old-versions': {
            type: 'update',
            query: '# WARNING: This will delete all cube versions ranked > 2 (keeps newest 2)\n# For safe deletion with backups, use the Deletion Wizard instead.\n#\n# This query deletes cubes and their related data for versions older than\n# the newest 2 versions of each base cube.\n#\n# IMPORTANT: Stardog requires separate DELETE and WHERE clauses.\n# The DELETE WHERE { ... } shorthand does not work with FILTER.\n\nPREFIX cube: <https://cube.link/>\nPREFIX xsd: <http://www.w3.org/2001/XMLSchema#>\n\n# Step 1: First run the Preview Deletions query to see what will be deleted\n# Step 2: Then modify this query with specific version URIs to delete\n\n# Example: Delete specific versions by URI pattern (safer approach)\nDELETE {\n  GRAPH <' + graphUri + '> {\n    ?s ?p ?o .\n  }\n}\nWHERE {\n  GRAPH <' + graphUri + '> {\n    # Match cube versions to delete (modify the FILTER pattern as needed)\n    ?cube a cube:Cube .\n    FILTER(REGEX(STR(?cube), "/version/[0-9]+$"))\n    \n    # Get version number and filter for versions to delete\n    BIND(xsd:integer(REPLACE(STR(?cube), "^.*/([0-9]+)$", "$1")) AS ?version)\n    \n    # Delete cubes with version number <= (max_version - 2)\n    # Adjust the number based on your data\n    # FILTER(?version <= YOUR_THRESHOLD_VERSION)\n    \n    # Match cube and its related observation sets and observations\n    {\n      ?s ?p ?o .\n      FILTER(?s = ?cube)\n    }\n    UNION\n    {\n      ?cube cube:observationSet ?set .\n      ?s ?p ?o .\n      FILTER(?s = ?set)\n    }\n    UNION\n    {\n      ?cube cube:observationSet ?set .\n      ?set cube:observation ?obs .\n      ?s ?p ?o .\n      FILTER(?s = ?obs)\n    }\n  }\n}'
        },
        'delete-orphans': {
            type: 'update',
